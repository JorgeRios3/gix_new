...
def asignaForce(HOST, LOCAL, PORT, INSTANCE, RPYC, WEB, TEST, GCMEX, SCROLL, SMART, QUERYONLY, POSTGRES):
	...
	global SMARTICS; global FORCEQUERYONLY; global FORCEPOSTGRES
	...
	SMARTICS = SMART; FORCEQUERYONLY = QUERYONLY; FORCEPOSTGRES=POSTGRES
...
class WxWidget2Excel:
	...
	def SetExcelWorkbook( self, excel = "aexcel.xls"):
		...
		if self.onlyexcel:
			...
			if dlg.ShowModal() == wx.ID_OK:
				...
				todobien = False
				#Mensajes().Info( self.parent, u"Grabar� en el archivo de default\n%s" % excel, u"Atenci�n")
		elif Mensajes().YesNo( self.parent, u"� Desea elegir el archivo de excel a grabar ?", u"Confirmaci�n"):
			if wx.Platform == '__WXMSW__':
				key = _winreg.OpenKey( _winreg.HKEY_CURRENT_USER,  "Software\\Microsoft\\Windows\\CurrentVersion\Explorer\\Shell Folders")
				ruta = _winreg.QueryValueEx( key, 'Personal')[0]
				_winreg.CloseKey( key )
	...
	def GenerateExcelFileFromListCtrl(self):
		...
		if not self.GetTemporalFile():
			Mensajes().Info( self.parent, u"No puedo generar el archivo de excel", u"Atenci�n")
			return
		...
		if not self.Rename():
			Mensajes().Info(self.parent, "El archivo de excel no pudo ser procesado", u"Atenci�n")
	...
	def GenerateExcelFileFromGrid(self):
		if wx.Platform == '__WXMSW__':
			...
			try:
				...
				Mensajes().Info(self.parent, u"Por favor instale el m�dulo pyExcelerator", u"Atenci�n")
	...
	def GenerateWithXl(self):
		...
		if not self.Rename():
			Mensajes().Info(self.parent, u"El archivo de excel no pudo ser procesado.", u"Atenci�n")
...
class Mensajes:
	def YesNo(self, parent, question, caption = u"Confirmaci�n"):
		try:
...
class GixDragController(wx.Control):
	""" Clase gen�rica para hacer uso del drag relacionando un control personalizado
	"""
...
class GixBase(object):
	...
	def DisplaySearchMenu(self, opciones):
		...
		dlg = wx.SingleChoiceDialog(
				self, u'Elija Tipo de Dato para la Busqueda', u'B�squeda por...',
	...
	def DisplaySearchResults(self, tipobusqueda = "...", listaParaBuscar = []):
		mf = wx.Frame(self, -1, u"Resultados de B�squeda por %s" % tipobusqueda , 
	...
	def RelatedFieldSearch(self, titulo, query, control, modal_llama = False, width = 550, height = 270):
		...
		def SelectionDone(event):
			if self.itemselection is None:
				Mensajes().Info(self,u"No ha escogido a�n", u"Atenci�n")
			else:
	...
	def OnLBox( self,event):
		...
		if self.datointernoynombre:
			...
			Mensajes().Warn(self, u"Escoja un puesto v�lido", u"Atenci�n")
	...
	def OnNewRecord(self, event):
		...
		if self.prerequisiciones:
			...
			if row is None:
				Mensajes().Info(self, u"Para registrar la prerequisici�n es necesario que exista el empleado %s.\n� No puede registrar la prerequisici�n !" % self.empleado, u"Atenci�n")
				self.EndModal(1)
				self.Destroy()
				return
			...
			if row is None:
				Mensajes().Info(self, u"Para registrar la prerequisici�n es necesario que se le asigne un puesto al empleado:\n%s\n� No puede registrar la prerequisici�n !"
	...
	def GetCurrentDate(self):
		""" Obtiene un tuple de dia mes y a�o con la fecha de hoy segun el db engine
		"""
		cursor = r_cn.cursor()
	...
	def QueryUpdateRecord(self, sql, sqlmax = "", insert = False, conexion = ""):
		...
		try:
			...
			conexion.rollback()
			Mensajes().Info(self, u"Problemas con \n%s\n\nConexion\n%s\n\nr_cn\n%s" % (sqlencoded, conexion, r_cn), u"Atenci�n")
			return False, ""
	...
	def OnFechaButton( self , event ):
		...
		o que acompa�a a un textctrl relativo a fecha
...
class GixBaseListCtrl(object):
	...
	def OpenBlogPopupMenu(self, idregistryblog = "", idtitleblog = "Registro", consulta = False):
		...
		if idtitleblog not in  ("Movimiento", "Prospecto"):
			popup.Append(ID_MOVTOPARTIDAS, u"Afectaci�n de Partidas del Ingreso %s" % self.datointerno)
	...
	def OnViewLogProspecto(self, event):
		goal = ((90, u"Fecha"), (90, u"Hora"), (90, u"A la Vista"), (100, u"Usuario"), (300, u"Gerente"), (300, u"Vendedor"),
		        (300, u"Prospecto"), (90, u"Nacimiento"), (150, u"R.F.C."), (150, u"C.U.R.P."), (100, u"Tel. C�sa"),
	...
	def UpdateBalance(self, cu, fechaingreso, cantidad, partidaid, tipomovto):
		...
		try:
			...
			Mensajes().Info(self, u"Problemas con la fecha al actualizar saldos\n" \
							...
							% (partidaid, fechaingreso, periodo), u"Atenci�n")
			updateok = False
			return updateok
...
class GixFrameCatalogo(wx.Frame):
	...
	def OnPrint(self,event):
		...
		if self.onlyexcel:
			...
			if Mensajes().YesNo(self, u"Desea adem�s archivo de Excel",u"Atenci�n"):
				self.HazExcel()
			wx.BeginBusyCursor()
	...
	def Nosepuede( self, event):
		Mensajes().Info(self,u"Ya no puede volver a intentar importaci�n\n" \
				u"Cierre la tabla ( Grid ) y vuelva a pedir\n" \
				u"la opci�n desde el men� principal", u"Atenci�n")
...
class GixProductoIntervaloFechasDialog(wx.Dialog, GixBase):
	...
	def OnBuscarProducto(self,event):
		...
		self.RelatedFieldSearch(u"B�squeda de Producto", query, ID_TEXTCTRLMRPRODUCTO)
...
class GixCatalogoGridBase(gridlib.Grid, GixBase, GixContabilidad):
	...
	def OnRightClick(self,event):
		...
		if Mensajes().YesNo(self,"Desea eliminar la columna %s" % (nombre,),u"Atenci�n"):
